...
beforeEach(() => {
    render(
        <Provider store={store}>
            <Router>
                <UserResetPasswordConfirm match={match} />
            </Router>
        </Provider>
    );
});
...
describe('submit button', () => {
    ...
    test('submitting form should dispatch resetPasswordConfirmAction', async () => {
        ...
        expect(await resetPasswordConfirmAction.mock.calls[0][0].uid).toBe(match.params.uid);
        expect(await resetPasswordConfirmAction.mock.calls[0][0].token).toBe(match.params.token);
    });
});
